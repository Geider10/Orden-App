...
const tituloError = d.getElementById("tituloError");
const bodyTablero = d.getElementById("bodyTablero");

btnAgregar.addEventListener("click",()=>{
    checkTitulos() == "Lista"? createList():createTask();
    iptTitulo.value="";
    btnAgregar.classList.add("off");
    tituloError.classList.remove("off");
})
const createTask=()=>{
...
const inputError = ()=>{
    iptTitulo.addEventListener("input",()=>{
        if(checkInput(iptTitulo).length>=3){
            btnAgregar.classList.remove("off");
            tituloError.classList.add("off");
        }
        else{
            btnAgregar.classList.add("off");
            tituloError.classList.remove("off");
...
const addSortListas =()=>{
    ...
    new Sortable(bodyTablero,{sort:"true",handle:".tablero__lista"});
}
